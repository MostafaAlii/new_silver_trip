...
class CaptainController extends Controller
{
    ...
    public function show($captainId) {
        try {
            ...
            $whatsappMedia = $this->getwhatsappMedia($data['captain']->phone);
            $findCaptions = CaptainProfile::where('uuid', $captainId)->first();
            $check = Captain::where('id', optional($findCaptions)->captain_id)->first();
            if ($check) {
                $checkStatus = optional(get_user_data())->type == "manager";
                if ($checkStatus) {
                    return view('dashboard.call-center.captains.show', compact('data', 'whatsappMedia'));
                }
    ...
    /************************************************* */
    public function getwhatsappMedia($phone) {
        if (substr($phone, 0, 1) !== '2') 
            $phone = '2' . $phone;
        if (substr($phone, 1, 1) !== '0') 
            $phone = substr_replace($phone, '0', 1, 0);
        $data = ['phone' => $phone];
        $response = Http::post('https://whatsapp.tripu.net/public/api/getPhoto', $data);
        if ($response->successful()) {
            return $response->json();
        }
        return redirect()->back()->with('error', 'Failed to fetch WhatsApp Media');
    }
