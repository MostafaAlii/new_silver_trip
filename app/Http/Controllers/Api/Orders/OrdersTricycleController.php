...
use App\Models\OrderTricycle;
use App\Models\OrderTakingTricycle;
use App\Models\OrderCanselTricycle;
use App\Models\UserProfile;
use App\Models\CaptainProfile;
use App\Models\CaptionActivity;
use Carbon\Carbon;
use App\Models\Traits\Api\ApiResponseTrait;
use App\Http\Resources\Orders\OrdersTricycleResources;
use Illuminate\Support\Facades\Validator;

class OrdersTricycleController extends Controller
{
    ...
    public function OrderExiting(Request $request)
    {
        ...
        $orderQuery = OrderTricycle::whereNotIn('status', ['done', 'cancel'])->latest();
    ...
    public function index(Request $request)
    {
        ...
        try {
            $order = OrderTricycle::where('order_code', $request->order_code)->firstOrFail();
            return $this->successResponse(new OrdersTricycleResources($order), 'Data created successfully');
    ...
    public function store(Request $request)
    {
        ...
        if (OrderTricycle::where('user_id', $request->user_id)->where('status', 'pending')->exists()) {
            return $this->errorResponse('This client is already on a journey');
        }

        if (OrderTricycle::where('captain_id', $request->captain_id)->where('status', 'pending')->exists()) {
            return $this->errorResponse('This captain is already on a journey');
        }

        try {
            $latestOrderId = optional(OrderTricycle::latest()->first())->id;
            ...
            if ($data) {
                ...
                createInFirebaseTricycle($request->user_id, $request->captain_id, $data->id);
            ...
            return $this->successResponse(new OrdersTricycleResources($data), 'Data created successfully');
        } catch (\Exception $exception) {
            return $this->errorResponse('Something went wrong, please try again later');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            $order = OrderTricycle::where('order_code', $request->order_code)->first();
            if (!$order) {
            ...
            return $this->successResponse(new OrdersTricycleResources($order), 'Data updated successfully');
        } catch (\Exception $exception) {
            return $this->errorResponse('Something went wrong, please try again later');
        }
    }
    ...
    private function completeOrder($order, $type)
    {
        ...
        DeletedInFirebaseTricycle($order->user_id, $order->captain_id, $order->id);
    }
    ...
    public function cancelOrder(Request $request)
    {
        ...
        $findOrder = OrderTricycle::where('order_code', $request->order_code)->first();
        $canselModel = OrderCanselTricycle::class;
        $firebaseDeletion = 'DeletedInFirebaseTricycle';
        $resourceType = OrdersTricycleResources::class;
    ...
    public function checkOrder(Request $request)
    {
        ...
        $checkOrder = OrderTricycle::where('order_code', $request->order_code)->first();

        if (!$checkOrder) {
            return $this->errorResponse('Order Code does not exist', 404);
        }

        return $this->successResponse(new OrdersTricycleResources($checkOrder), 'Data returned successfully');
    }
    ...
    public function takingOrder(Request $request)
    {
        ...
        OrderTakingTricycle::where('order_scooter_id', $findOrder->id)->update([
            'lat_caption' => $request->lat_caption,
            'long_caption' => $request->long_caption,
        ]);
    ...
    {
        $findOrder = OrderTricycle::where('order_code', $order_code)->first();
        if ($findOrder) {
            CaptionActivity::where('captain_id', $findOrder->captain_id)->update([
            ...
            OrderTakingTricycle::where('order_scooter_id', $findOrder->id)->delete();
