...
use App\Models\User;
use App\Models\OrderTrunk;
use App\Models\OrderTakingTrunk;
use App\Models\OrderCanselTrunk;
use Carbon\Carbon;
...
use App\Http\Resources\Orders\OrdersTrunkResources;

class OrdersTrunkController extends Controller
{
    ...
    public function OrderExiting(Request $request)
    {
        ...
        $orderQuery = OrderTrunk::whereNotIn('status', ['done', 'cancel'])->latest();
    ...
    public function index(Request $request)
    {
        ...
        try {
            $order = OrderTrunk::where('order_code', $request->order_code)->firstOrFail();
            return $this->successResponse(new OrdersTrunkResources($order), 'Data created successfully');
    ...
    public function store(Request $request)
    {
        ...
        if (OrderTrunk::where('user_id', $request->user_id)->where('status', 'pending')->exists()) {
            return $this->errorResponse('This client is already on a journey');
        }

        if (OrderTrunk::where('captain_id', $request->captain_id)->where('status', 'pending')->exists()) {
            return $this->errorResponse('This captain is already on a journey');
        }

        try {
            $latestOrderId = optional(OrderTrunk::latest()->first())->id;
            ...
            if ($data) {
                ...
                createInFirebaseTrunk($request->user_id, $request->captain_id, $data->id);
            ...
            return $this->successResponse(new OrdersTrunkResources($data), 'Data created successfully');
        } catch (\Exception $exception) {
            return $this->errorResponse('Something went wrong, please try again later');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            $order = OrderTrunk::where('order_code', $request->order_code)->first();
            if (!$order) {
            ...
            return $this->successResponse(new OrdersTrunkResources($order), 'Data updated successfully');
        } catch (\Exception $exception) {
            return $this->errorResponse('Something went wrong, please try again later');
        }
    }
    ...
    private function completeOrder($order, $type)
    {
        ...
        DeletedInFirebaseTrunk($order->user_id, $order->captain_id, $order->id);
    }
    ...
    public function cancelOrder(Request $request)
    {
        ...
        $findOrder = OrderTrunk::where('order_code', $request->order_code)->first();
        $canselModel = OrderCanselTrunk::class;
        $firebaseDeletion = 'DeletedInFirebaseTricycle';
        $resourceType = OrdersTrunkResources::class;
    ...
    public function checkOrder(Request $request)
    {
        ...
        $checkOrder = OrderTrunk::where('order_code', $request->order_code)->first();

        if (!$checkOrder) {
            return $this->errorResponse('Order Code does not exist', 404);
        }

        return $this->successResponse(new OrdersTrunkResources($checkOrder), 'Data returned successfully');
    }
    ...
    public function takingOrder(Request $request)
    {
        ...
        OrderTakingTrunk::where('order_scooter_id', $findOrder->id)->update([
            'lat_caption' => $request->lat_caption,
            'long_caption' => $request->long_caption,
        ]);
    ...
    {
        $findOrder = OrderTrunk::where('order_code', $order_code)->first();
        if ($findOrder) {
            CaptionActivity::where('captain_id', $findOrder->captain_id)->update([
            ...
            OrderTakingTrunk::where('order_scooter_id', $findOrder->id)->delete();
